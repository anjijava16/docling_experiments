...
def extract_pdf_with_docling(pdf_source: str):
    ...
    pipeline_options.generate_table_images = True  # Enable table extraction
...
def extract_tables(result) -> List[Dict]:
    ...
    for element, _level in result.document.iterate_items():
        if isinstance(element, TableItem):
            ...
            # Also get table as text (for LLM analysis)
            ...
            tables.append({
                ...
                'text': table_text,
                'caption': element.caption.text if element.caption else '',
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("PDF TO CHROMADB - IMAGES AND TABLES")
    print("="*80)
    ...
    print("""
# Import
from your_script import ingest_pdf, search_similar

# Ingest PDF (extracts images AND tables)
stats = ingest_pdf("document.pdf")
print(f"Processed {stats['images_found']} images and {stats['tables_found']} tables")

# Search
results = search_similar("What do the tables show?", k=5)
